...
export function UnstakeSection(props: {vault: TVaultData; onRefreshVaultData: () => void}): ReactElement {
	const {vault, onRefreshVaultData} = props;
	...
	const onUnstake = useCallback(async (): Promise<void> => {
		let actualValue = amount;

		if (isMax) {
			actualValue = actualBalanceInToken || zeroNormalizedBN;
		}
		...
		const result = await redeemV3Shares({
			...
			amount: toBigInt(actualValue?.raw),
			contractAddress: vault.autoCompoundingAddress,
			statusHandler: set_withdrawStatus
		});
		if (result.isSuccessful) {
			onRefreshVaultData();
			set_amount(undefined);
			toast.success(`${vault.tokenSymbol} successfully unstaked.`);
		}
	}, [amount, isMax, provider, actualBalanceInToken, onRefreshVaultData, vault]);
		...
		<div className={'pb-2 md:pb-[92px]'}>
			...
			<div className={'flex flex-col gap-2 md:flex-row'}>
				<input
					...
					onChange={e => {
						set_isMax(false);
						set_amount(onInput(e, vault.decimals, actualBalanceInToken));
					}}
				/>
				<button
					onClick={onUnstake}
			...
			<button
				...
				className={'mt-1 block pl-2 text-xs text-neutral-900'}>
				{`You have ${formatAmount(actualBalanceInToken?.normalized || 0, 4)} ${vault.tokenSymbol}`}
			</button>
		</div>
	);
}
		...
		<div className={'hidden grid-cols-2 gap-4 pt-0.5 md:grid'}>
			...
			<div
				...
				<b
					className={'block '}
					suppressHydrationWarning>
					{`${formatAmount(actualBalanceInToken.normalized, 4)} ${props.vault.tokenSymbol}`}
				</b>
			</div>
		</div>
	);
